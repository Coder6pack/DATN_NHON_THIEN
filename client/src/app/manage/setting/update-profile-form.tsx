...
import { UpdateMeBodySchema, UpdateMeBodyType } from '@/schemaValidations/profile.model'
import { useState, useRef, useEffect, useMemo } from 'react'
import { useAccountMe, useUpdateAccountProfileMutation } from '@/app/queries/useAccount'
...
export default function UpdateProfileForm() {
  ...
  useEffect(() => {
    if(data){
      const {avatar,name} = data.payload
      form.reset({
        name,
        avatar: avatar ?? ''
      })
...
const previewAvatar = file ? URL.createObjectURL(file) : avatar;
  return (
    <Form {...form}>
      <form noValidate className='grid auto-rows-max items-start gap-4 md:gap-8' onReset={reset} onSubmit={form.handleSubmit(onSubmit,e => console.log(e))}>
